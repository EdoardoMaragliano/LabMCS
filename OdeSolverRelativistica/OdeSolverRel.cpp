...
void OdeSolverRel::Punto(PuntoMateriale tmp){
  m_p.push_back(tmp);
}

vector<PuntoMateriale> OdeSolverRel::Punti(){
  return m_p;
}

PuntoMateriale OdeSolverRel::Punto(unsigned int i){
  return m_p[i];
}

unsigned int OdeSolverRel::N(){
  return m_p.size();
}

void OdeSolverRel::T(double t0){
  m_t=t0;
}

double OdeSolverRel::T(){
  return      m_t;
}

void OdeSolverRel::Passo(double h){
  m_h = h;
}

double OdeSolverRel::Passo(){
  return   m_h;
}

//STEP 3 Calcolo dell'accelerazione dovuta a forze interne e forze esterne
Vettore OdeSolverRel::m_eqDiff(unsigned int i, double t, vector<PuntoMateriale> p){
  Vettore sum(0,0,0);
