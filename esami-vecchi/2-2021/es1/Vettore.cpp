...
Vettore Vettore::Vers() {   //updated 31 gen 2021
  if (this->Mod()!=0)
    return (*this)*(1/Mod());
  else 
...
Vettore Vettore::Pow(double p){   //updated 31 gen 2021
  double vx=pow(this->X(),p);
  double vy=pow(this->Y(),p);
  double vz=pow(this->Z(),p);
