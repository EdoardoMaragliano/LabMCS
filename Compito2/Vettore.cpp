...
double Vettore::operator*( Vettore b){    //scalar product
  double prod=0;
  for (int i=0;i<3;i++){
    prod += m_v[i]*b.m_v[i];
  }
...
Vettore Vettore::operator^( Vettore b){  //vector product
  Vettore res;
...
Vettore Vettore::operator*(double f){   // product by scalar
  Vettore res;
  for (int i=0;i<3;i++){
    res.m_v[i] = m_v[i]*f;
  }
...
Vettore operator*(double f,  Vettore b){  // product by scalar
  return b*f;
}
...
Vettore Vettore::Pow(const Vettore v, double p){
  double vx=pow(v.X(),p);
  double vy=pow(v.Y(),p);
  double vz=pow(v.Z(),p);
  return Vettore(vx,vy,vz);
}
